        ...
        <div className="bg-[#0F0F0F] rounded-lg p-5 sm:p-6 border border-[#1A1A1A]">
          ...
          {isLoading ? (
            ...
            <div className="space-y-6">
                    ...
                    {(!isSubscribed || subscriptionTier === 'pro') && (
                      ...
                      >
                        <span className="flex items-center justify-center gap-1">
                          ...
                          {isSubscribed ? 'Upgrade to Premium' : 'Subscribe to Premium'}
                        </span>
                      </ButtonCheckout>
                    )}
              ...
              {/* Plan Comparison - Only show if user is not subscribed */}
              {!isSubscribed && (
                <div className="pt-5 mt-2 border-t border-[#1A1A1A]">
                  <h3 className="text-sm font-medium text-white mb-4">Plan Comparison</h3>
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div className="bg-zinc-900/50 p-5 rounded-lg border border-[#1A1A1A]">
                      ...
                      {proPlan.description && (
                        <p className="text-sm text-white/80 mb-4">{proPlan.description}</p>
                      )}
                      <ul className="space-y-2.5 text-sm">
                        {proPlan.features && proPlan.features.map((feature, i) => (
                          <li key={i} className="flex items-start gap-2">
                            ...
                            <span className="text-gray-300">{typeof feature === 'string' ? feature : feature.name}</span>
                          </li>
                        ))}
                      </ul>
                    </div>
                    ...
                    <div className="bg-red-500/10 p-5 rounded-lg border border-red-500/20">
                      ...
                      {premiumPlan.description && (
                        <p className="text-sm text-white/80 mb-4">{premiumPlan.description}</p>
                      )}
                      <ul className="space-y-2.5 text-sm">
                        {premiumPlan.features && premiumPlan.features.map((feature, i) => (
                          <li key={i} className="flex items-start gap-2">
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              viewBox="0 0 20 20"
