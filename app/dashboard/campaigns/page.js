    ...
    <div className="space-y-6">
      ...
      {!analyticsData.isLoading && (
        <div className="space-y-4">
          {/* Active Campaigns */}
          {(activeTab === "all" || activeTab === "active") && campaigns.active.length > 0 && (
                    ...
                    <div key={campaign.job_id} className="flex items-center justify-between p-4 bg-black rounded-lg border border-[#1A1A1A]">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-[#A1A1AA]">{campaign.total_invitations} prospects • {Math.min(100, campaign.accepted_connections / campaign.total_invitations * 100).toFixed(0)}% complete</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
          ...
          {/* Draft Campaigns */}
          {(activeTab === "all" || activeTab === "drafts") && campaigns.drafts.length > 0 && (
            <div className="bg-[#0F0F0F] rounded-lg border border-[#1A1A1A]">
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          ...
                          <p className="text-sm text-[#A1A1AA]">
                            Draft • Last edited {campaign.created_at ? new Date(campaign.created_at).toLocaleDateString() : "recently"}
                          </p>
                        </div>
                      </div>
          ...
          {/* Completed Campaigns */}
          {(activeTab === "all" || activeTab === "completed") && campaigns.completed.length > 0 && (
            <div className="bg-[#0F0F0F] rounded-lg border border-[#1A1A1A]">
              <div className="p-6">
                <div className="flex items-center justify-between mb-4">
                  <div>
                  ...
                  {campaigns.completed.map(campaign => (
                    <div key={campaign.job_id} className="flex items-center justify-between p-4 bg-black rounded-lg border border-[#1A1A1A]">
                      <div className="flex items-center gap-4">
                        ...
                        <div>
                          <h4 className="text-white font-medium">{campaign.name}</h4>
                          <p className="text-sm text-[#A1A1AA]">{campaign.total_invitations} prospects • Completed</p>
                        </div>
                      </div>
                      <div className="flex items-center gap-4">
                        <div className="text-right">
          ...
          {!analyticsData.isLoading && 
            ...
            <div className="bg-[#0F0F0F] rounded-lg border border-[#1A1A1A] p-12 text-center">
              <p className="text-[#A1A1AA] mb-4">You don&apos;t have any campaigns yet</p>
              <Button
                onClick={() => router.push('/dashboard/campaigns/new/leads')}
              >
